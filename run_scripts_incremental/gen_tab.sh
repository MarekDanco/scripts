...
function seq_of { # SOLUTION_FILE N_QUERIES
  # ordered answer stream as letters: S=sat U=unsat X=unknown E=error,
  # padded with '-' for queries left unanswered (timeout / crash).
  s=`grep -E -w -e '^sat' -e '^unsat' -e '^unknown' -e '^\(error' "${1}" \
       | sed -E 's/^sat$/S/; s/^unsat$/U/; s/^unknown$/X/; s/^\(error.*/E/' \
       | tr -d '\n'`
...
do
  ...
  NQ=`grep -c -E '^\(check-sat' ${FF}`
  SEQ=`seq_of ${SOLUTION_FILE} ${NQ}`
